...
We achieve a final public leaderboard score of 16.87/100.
...
The provided training dataset contains 50,000 (prompt, SVG) pairs. Prompts are short natural language descriptions (mean $\sim$100 characters, $\sim$30 tokens) of icons and simple scenes. SVGs range from 91 to 15,937 characters (median 2,110), with 82\% using a \texttt{viewBox} of \texttt{0 0 200 200}.
...
Initial fine-tuning with rank-16 LoRA adapters on 45,874 competition samples for 3 epochs. This established our first baseline (Kaggle score: 15.47). Training used 4-bit NF4 quantization via Unsloth \citep{unsloth2024}, \texttt{paged\_adamw\_8bit} optimizer, learning rate $2 \times 10^{-4}$ with cosine schedule.

\textbf{Round 2 --- Merge + LoRA r=32 (kepler RTX 2000 Ada):}
We merge the Round 1 adapter into the base weights, permanently baking the Round 1 knowledge into the model parameters. A fresh rank-32 LoRA adapter (2$\times$ trainable parameters: 37M vs.\ 18M) is then applied to the merged model. This was trained on 45,000 competition-only samples for 2 epochs. Score: \textbf{16.87} --- our best result, representing a +1.4 point improvement from the merge-and-retrain technique alone.
...
Table~\ref{tab:kaggle-results} shows all 9 Kaggle submissions. Our best public leaderboard score was 16.87.
...
\midrule
LoRA r=16, 3ep & 15.47 & First submission \\
r=32, 76k mixed & 14.64 & External data hurt \\
r=32, 45k comp & \textbf{16.87} & \textbf{Best public score} \\
...
{\small
\begin{align}
V_i &= 0.7 \cdot \text{SSIM}(I_p, I_g) \nonumber \\
    &\quad + 0.3 \cdot \text{EdgeF1}(I_p, I_g) \\
S_i &= \exp\!\left(-\tfrac{\text{TED}(A_p, A_g)}{25}\right) \\
C_i &= \exp\!\left(-\left|\log\tfrac{\text{len}(p_i) + 50}{\text{len}(g_i) + 50}\right|\right) \\
Q_i &= V_i^{0.85} \cdot S_i^{0.12} \cdot C_i^{0.03}
\end{align}
}
where $I_p, I_g$ render predicted/reference SVGs to 256$\times$256 grayscale images and $A_p, A_g$ extract XML tree structures.
...
All code and model weights are publicly available:

\begin{itemize}
    \item \textbf{GitHub Repository:} \url{https://github.com/ivanearisty/svg-gen} --- training scripts, inference pipelines, ablation automation, preprocessing, scoring, and this report's \LaTeX{} source.
    \item \textbf{Model Weights (HuggingFace):} \url{https://huggingface.co/kaleidoscopicwhether/svg-gen-weights} --- all trained models listed below.
\end{itemize}

\noindent Table~\ref{tab:model-artifacts} lists all model artifacts with direct links.

\begin{table}[h]
\centering
\small
\begin{tabular}{p{2.8cm}p{1.2cm}r}
\toprule
\textbf{Model} & \textbf{Type} & \textbf{Kaggle} \\
\midrule
\href{https://huggingface.co/kaleidoscopicwhether/svg-gen-weights/tree/main/componly-r32-adapter}{\texttt{componly-r32-adapter}} & Adapter & \textbf{16.87} \\
\href{https://huggingface.co/kaleidoscopicwhether/svg-gen-weights/tree/main/merged-1.5b-r16}{\texttt{merged-1.5b-r16}} & Full & (base) \\
\href{https://huggingface.co/kaleidoscopicwhether/svg-gen-weights/tree/main/refined-7000}{\texttt{refined-7000}} & Full & 16.26 \\
\href{https://huggingface.co/kaleidoscopicwhether/svg-gen-weights/tree/main/r16-3epoch}{\texttt{r16-3epoch}} & Adapter & 15.47 \\
\href{https://huggingface.co/kaleidoscopicwhether/svg-gen-weights/tree/main/mixed-r32-adapter}{\texttt{mixed-r32-adapter}} & Adapter & 14.64 \\
\href{https://huggingface.co/kaleidoscopicwhether/svg-gen-weights/tree/main/codegen-1.5b}{\texttt{codegen-1.5b}} & Full & 12.26 \\
\bottomrule
\end{tabular}
\caption{Model artifacts on HuggingFace. The best model (\texttt{componly-r32-adapter}) requires \texttt{merged-1.5b-r16} as its base. All other full models are standalone.}
